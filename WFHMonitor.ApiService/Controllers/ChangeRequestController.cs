...
public class ChangeRequestController : Controller
{
    ...
    private readonly IAiAutomationService _aiAutomation;
    ...
    public ChangeRequestController(
        ...
        IAiAutomationService aiAutomation,
    ...
    public async Task<IActionResult> FeatureDetails(int featureId)
    {
        ...
        ViewBag.IsAiAutomationEnabled =
            planSettings.EnableAiAutomation;
        ViewBag.HasProAccess = currentUser is not null
            && HasAiAutomationAccess(currentUser, planSettings);
        ViewBag.AiAutomationFeatureProfileOptions =
            GetAiAutomationFeatureProfileOptions();
    ...
    public async Task<IActionResult> PickupFeature(int featureId, string? featureAgentId, string? assignedAgentId, string? returnUrl)
    {
        ...
        var planSettings = await _systemSettingsService.GetProVersionSettingsAsync();
        if (!planSettings.EnableAiAutomation)
        {
            TempData["Error"] =
                "AI automation is temporarily disabled by admin.";
        ...
        {
            TempData["Error"] =
                "AI feature guidance is available for administrators on the Pro plan only.";
            return RedirectToAction("Index", "Payment");
        }
        ...
        IReadOnlyList<string> configuredFeatureAgentIds = ["codex"];
        ...
        {
            TempData["Error"] =
                "Selected AI automation profile is not allowed by configuration.";
            return RedirectFeatureLocal(returnUrl, featureId);
        }
        ...
        TempData["Success"] = wasAgentProcessing
            ? $"AI feature guidance re-queued with '{selectedFeatureAgentId}'."
            : $"AI feature guidance queued with '{selectedFeatureAgentId}'.";
        return RedirectFeatureLocal(returnUrl, featureId);
    }
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        ViewBag.AiAutomationScanProfileOptions =
            GetAiAutomationScanProfileOptions();
        var currentUser = await _userManager.GetUserAsync(User);
        var planSettings = await _systemSettingsService.GetProVersionSettingsAsync();
        ViewBag.IsAiAutomationEnabled =
            planSettings.EnableAiAutomation;
        ...
        var canUseAiAutomationHealth =
            planSettings.EnableAiAutomation &&
            currentUser is not null &&
            HasAiAutomationAccess(currentUser, planSettings);
        ViewBag.CanUseAiAutomationHealth =
            canUseAiAutomationHealth;
        ViewBag.HealthAiAutomationProfileId =
            cr.BugScanAgentId;
        ...
        if (cr.ProjectHealthScore.HasValue && !string.IsNullOrWhiteSpace(cr.ProjectHealthLabel))
        {
            fallbackHealth = new ProjectHealthViewModel
            {
                ...
                UsedAiAutomation = true
            };
        }
        else if (canUseAiAutomationHealth)
        {
            fallbackHealth.Summary =
                "Health score shown from live signals. Click Analyze to run AI automation and enrich insights.";
        }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        var cr = await _db.ChangeRequests
            ...
            .FirstOrDefaultAsync(c => c.Id == id);
        if (cr == null) return NotFound();
        if (!await CanViewProjectAsync(cr.Id))
            return Forbid();
    ...
    public async Task<IActionResult> DeleteImage(int imageId, int crId)
    {
        ...
        if (img.ChangeRequestId != crId || !await CanViewProjectAsync(img.ChangeRequestId))
            return Forbid();
    ...
    public async Task<IActionResult> DeleteDocument(int documentId, int crId)
    {
        ...
        if (doc.ChangeRequestId != crId || !await CanViewProjectAsync(doc.ChangeRequestId))
            return Forbid();
    ...
    public async Task<IActionResult> FindBugs(int id, string? scanAgentId)
    {
        ...
        {
            TempData["Error"] =
                "AI Find Bugs is available for administrators on the Pro plan only.";
            return RedirectToAction("Index", "Payment");
        }

        IReadOnlyList<string> configuredScanAgentIds = ["codex"];
    ...
    public async Task<IActionResult> AnalyzeHealth(int id, string? scanAgentId)
    {
        ...
        {
            TempData["Error"] =
                "AI Project Health is available for administrators on the Pro plan only.";
            return RedirectToAction("Index", "Payment");
        }
        ...
        try
        {
            var workspace = await _workspaceService.PrepareReadOnlyAsync(
                project,
                userId,
                $"health-{project.CrNumber}",
                HttpContext.RequestAborted);
            var healthResult = await _aiAutomation.AnalyzeProjectHealthAsync(
                project,
                totalBugCount,
                openBugCount,
                featureCount,
    ...
    private static bool HasAiAutomationAccess(
        ApplicationUser user,
        ProVersionSettingsViewModel settings) =>
        settings.EnableAiAutomation &&
        (HasActiveProAccess(user)
         || settings.AllowAiAutomationForFreePlan);
    ...
    private List<SelectListItem> GetAiAutomationScanProfileOptions()
    {
        IReadOnlyList<string> configured = ["codex"];
    ...
    private List<SelectListItem> GetAiAutomationFeatureProfileOptions()
    {
        IReadOnlyList<string> configured = ["codex"];
        if (configured.Count == 0)
            configured = ["main"];
