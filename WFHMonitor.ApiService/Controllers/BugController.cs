...
public class BugController : Controller
{
    ...
    public async Task<IActionResult> Details(int id)
    {
        try
        {
            ...
            ViewBag.AiAutomationFixProfileOptions =
                GetAiAutomationFixProfileOptions();
    ...
    public async Task<IActionResult> FixWithAgent(int id, string? fixAgentId, string? assignedAgentId)
    {
        ...
        {
            TempData["Error"] =
                "AI Fix Bug is available for administrators on the Pro plan only.";
            return RedirectToAction("Index", "Payment");
        }
        ...
        IReadOnlyList<string> configuredFixAgentIds = ["codex"];
        ...
        var assignToAgentId = string.IsNullOrWhiteSpace(assignedAgentId)
            ? await ResolveAutomationAssigneeAgentIdAsync()
        ...
        _db.BugActivities.Add(new BugActivity
        {
            ...
            Action = wasAgentProcessing
                ? $"AI automation fix manually re-queued ({selectedFixAgentId})"
                : $"AI automation fix queued ({selectedFixAgentId})",
            OldStatus = oldStatus,
        ...
        TempData["Success"] = wasAgentProcessing
            ? $"AI automation fix re-queued with '{selectedFixAgentId}'."
            : $"AI automation fix queued with '{selectedFixAgentId}'.";
        return RedirectToAction(nameof(Details), new { id });
    }
    ...
    {
        var accessResult = await EnsureBugAccessAsync(id);
    ...
    {
        var belongsToBug = await _db.BugScreenshots
            .AsNoTracking()
            .AnyAsync(item => item.Id == screenshotId && item.BugReportId == bugId);
        if (!belongsToBug)
            return NotFound();

        var accessResult = await EnsureBugAccessAsync(bugId);
        if (accessResult is not null)
            return accessResult;
    ...
    {
        var belongsToBug = await _db.BugDocuments
            .AsNoTracking()
            .AnyAsync(item => item.Id == documentId && item.BugReportId == bugId);
        if (!belongsToBug)
            return NotFound();
    ...
    private async Task<IActionResult?> EnsureBugAccessAsync(int bugId)
    {
        var bug = await _bugService.GetDetailsAsync(bugId);
        if (bug is null)
            return NotFound();

        var currentUser = await _userManager.GetUserAsync(User);
        return CanAccessBugByTeam(bug, currentUser) ? null : Forbid();
    }
    ...
    private List<SelectListItem> GetAiAutomationFixProfileOptions()
    {
    ...
    private async Task<string?> ResolveAutomationAssigneeAgentIdAsync()
    {
        var currentUser = await _userManager.GetUserAsync(User);
        var currentCompany = NormalizeCompanyName(currentUser?.CompanyName);
        var agents = (await _userRoleCache.GetUsersInRoleAsync("Agent"))
