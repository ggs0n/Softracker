...
public class QaController : Controller
{
    ...
    private readonly IQaAiAutomationQueueService _qaAiAutomationQueueService;

    public QaController(
        ApplicationDbContext db,
        UserManager<ApplicationUser> userManager,
        ISystemSettingsService settingsService,
        IQaAiAutomationQueueService qaAiAutomationQueueService)
    {
        _db = db;
        _userManager = userManager;
        _settingsService = settingsService;
        _qaAiAutomationQueueService = qaAiAutomationQueueService;
    }
    ...
    public async Task<IActionResult> Index(string? category, string? status, int? projectId, string? tab, string? scanAgentId)
    {
        ...
        var scanAgentOptions = new[] { "codex" }
        ...
        var vm = new QaIndexViewModel
        {
            ...
            AiAutomationProfileOptions = scanAgentOptions
        };
    ...
    public async Task<IActionResult> ScanAndGenerate(int projectId, string? scanAgentId, bool useSecurityPrompt = false)
    {
        ...
        try
        {
            await _qaAiAutomationQueueService.EnqueueAsync(
                new QaAiAutomationQueueItem(
                    QaAiAutomationQueueOperation.ScanAndGenerate,
                    userId,
                    ProjectId: projectId,
            ...
            TempData["Success"] = useSecurityPrompt
                ? "AI security scan queued. Bugs will be added in the background."
                : "AI scan queued. Bugs will be added in the background.";
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> AutoGenerate(int projectId, string? scanAgentId)
    {
        ...
        try
        {
            await _qaAiAutomationQueueService.EnqueueAsync(
                new QaAiAutomationQueueItem(
                    QaAiAutomationQueueOperation.AutoGenerate,
                    userId,
                    ProjectId: projectId,
                    ScanAgentId: scanAgentId),
                HttpContext.RequestAborted);
            ...
            TempData["Success"] =
                "AI test generation queued. Test cases will be created in the background.";
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> GenerateFlow(int projectId, string? scanAgentId, string? mode)
    {
        ...
        try
        {
            ...
            if (normalizedMode == "generate_and_scan")
            {
                ...
                TempData["Success"] =
                    "AI automation queued: test generation and project scan.";
            }
            else
            {
                TempData["Success"] =
                    "AI automation queued: test generation only.";
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> RunSecurityScan(int projectId, string? scanAgentId, string? mode)
    {
        ...
        try
        {
            if (normalizedMode == "full_pass")
            {
                ...
                TempData["Success"] =
                    "AI security scan queued for a full project pass.";
            }
            else
            {
                ...
                foreach (var id in taggedCaseIds)
                {
                    await _qaAiAutomationQueueService.EnqueueAsync(
                        new QaAiAutomationQueueItem(
                            QaAiAutomationQueueOperation.ScanModule,
                            userId,
                            TestCaseId: id,
                            ScanAgentId: scanAgentId,
                            UseSecurityPrompt: true),
                ...
                TempData["Success"] =
                    $"AI security scan queued for {taggedCaseIds.Count} security-tagged test case(s).";
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> ScanModule(int id)
    {
        ...
        try
        {
            ...
            TempData["Success"] =
                "AI module scan queued. Bugs will be added in the background.";
        }
        catch (Exception ex)
        {
