...
public class GitHubService : IGitHubService
{
    ...
    private readonly Uri _webBaseUri;
        ...
        IGitHubOAuthService gitHubOAuthService)
    {
        ...
        _apiBaseUri = GetBaseUri(
            settings.Value.ApiBaseUrl,
            "GitHubSettings:ApiBaseUrl");
        _webBaseUri = GetBaseUri(
            settings.Value.WebBaseUrl,
            "GitHubSettings:WebBaseUrl");
    ...
    public async Task<List<GitHubCommit>> GetCommitsAsync(string owner, string repo, string branch, int count = 10)
    {
        var json = await SendGitHubGetAsync(
            GetApiUrl($"repos/{owner}/{repo}/commits?sha={branch}&per_page={count}"));
    ...
    {
        var json = await SendGitHubGetAsync(
            GetApiUrl($"repos/{owner}/{repo}/git/trees/{branch}?recursive=1"));
        using var doc = JsonDocument.Parse(json);
    ...
    public async Task<GitHubRepositoryInfo> GetRepositoryInfoAsync(string owner, string repo)
    {
        ...
        return new GitHubRepositoryInfo(
            ...
            root.GetProperty("html_url").GetString()
                ?? new Uri(_webBaseUri, $"{owner}/{repo}").ToString(),
    ...
    public async Task<string?> GetReadmeContentAsync(string owner, string repo)
    {
        var request = await CreateRequestAsync(
            HttpMethod.Get,
            GetApiUrl($"repos/{owner}/{repo}/readme"));
    ...
    {
        var json = await SendGitHubGetAsync(
            GetApiUrl($"repos/{owner}/{repo}/languages"));
    ...
    public async Task<List<GitHubConnectedRepository>> GetCurrentUserRepositoriesAsync(int maxCount = 200)
    {
        ...
        while (repositories.Count < maxCount && page <= maxPages)
        {
            ...
            var requestUrl = GetApiUrl(
                $"user/repos?visibility=all&affiliation=owner,collaborator,organization_member&sort=updated&per_page={requestedPageSize}&page={page}");
    ...
    public async Task<GitHubPullRequestResult> CreateDraftPullRequestAsync(
        string owner,
        string repository,
        string title,
        string headBranch,
    ...
    {
        var token =
            await _gitHubOAuthService.GetAccessTokenForUserAsync(
                requestingUserId);
        if (string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException(
                "Connect GitHub before creating a draft pull request.");

        using var request = new HttpRequestMessage(
            HttpMethod.Post,
            GetApiUrl($"repos/{owner}/{repository}/pulls"))
        {
        ...
        {
            throw new InvalidOperationException(
                $"GitHub draft PR creation failed with status {(int)response.StatusCode}.");
        }
    ...
    {
        return new Uri(_apiBaseUri, relativePath).ToString();
    }

    private static Uri GetBaseUri(string value, string configurationKey)
    {
        if (Uri.TryCreate(
                $"{value.TrimEnd('/')}/",
                UriKind.Absolute,
        ...
        throw new InvalidOperationException(
            $"{configurationKey} must be an absolute URL.");
    }
}
