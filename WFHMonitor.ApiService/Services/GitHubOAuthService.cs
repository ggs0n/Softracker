...
public class GitHubOAuthService : IGitHubOAuthService
{
    ...
    {
        var token = await GetAccessTokenForUserCoreAsync(userId);
    ...
    public async Task<string?> GetCurrentUserAccessTokenAsync()
    {
        ...
        return await GetAccessTokenForUserCoreAsync(userId);
    }

    public Task<string?> GetAccessTokenForUserAsync(string userId) =>
        GetAccessTokenForUserCoreAsync(userId);
    ...
    public Task<string> BuildAuthorizeUrlAsync(string userId, string? returnUrl)
    {
        ...
        var url = QueryHelpers.AddQueryString(_settings.AuthorizeUrl, query);
    ...
    private async Task<string?> GetAccessTokenForUserCoreAsync(string? userId)
    {
        if (string.IsNullOrWhiteSpace(userId))
            return null;
