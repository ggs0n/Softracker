    ...
    private async Task ProcessItemAsync(FeatureAgentQueueItem item, CancellationToken cancellationToken)
    {
        ...
        if (!settings.EnableAiAutomation)
        {
            ...
            feature.AgentImplementationPlan = AppendTextWithLimit(
                ...
                $"[AI Automation Feature Run Failed - Codex - {DateTime.UtcNow:yyyy-MM-dd HH:mm UTC}]\nAI automation is temporarily disabled by admin.",
                4000);
        ...
        var result = await automation.ImplementFeatureAsync(
            feature,
        ...
        feature.AgentImplementationPlan = AppendTextWithLimit(
            feature.AgentImplementationPlan,
            $"[AI Automation Feature Guidance - Codex - {DateTime.UtcNow:yyyy-MM-dd HH:mm UTC}]\n{result.ImplementationPlan.Trim()}",
            4000);
